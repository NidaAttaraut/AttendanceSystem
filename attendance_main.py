...
def mark_attendance(name, students, csv_writer, current_day):
    ...
    '''name = False
    with open(, 'r') as csvfile_read:
        csv_reader = csv.DictReader(csvfile_read)
        for row in csv_reader:
            if row["Full Name"] == name:
                # Name already exists, update the attendance for the current date
                row[f"{current_day}/{current_month}"] = 1
                name = True
                break'''
